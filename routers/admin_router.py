...
def medicos_pendientes(_u=Depends(admin_only), db: Session = Depends(get_db)):
    """Retorna medicos pendientes de aprobacion."""
    pendientes = db.query(Usuario).filter(
        Usuario.rol == "medico",
        Usuario.aprobado.is_(False)
    ).all()
    return [
        {
            "id": x.id,
...
def aprobar(uid: int, _u=Depends(admin_only), db: Session = Depends(get_db)):
    """Aprueba un medico pendiente."""
    return facade.admin.aprobar_medico(uid, db)


@router.put("/usuarios/{uid}/rechazar")
def rechazar(uid: int, _u=Depends(admin_only), db: Session = Depends(get_db)):
    """Rechaza y elimina un medico pendiente."""
...
def activar(uid: int, _u=Depends(admin_only), db: Session = Depends(get_db)):
    """Activa o desactiva un usuario."""
    x = db.query(Usuario).filter(Usuario.id == uid).first()
    if not x:
        raise HTTPException(404, "No encontrado")
    x.activo = not x.activo
    db.commit()
...
def estadisticas(_u=Depends(admin_only), db: Session = Depends(get_db)):
    """Retorna estadisticas generales del sistema."""
    num_pendientes = db.query(Usuario).filter(
        Usuario.rol == "medico",
        Usuario.aprobado.is_(False)
    ).count()
    ...
    return {
        ...
        "medicos_pendientes": num_pendientes,
