...
def get_tarifa(esp_id: int, db: Session = Depends(get_db)):
    ...
    ANTES: hardcodeada en citas.html L310-L318.
    ...
    if not esp: raise HTTPException(404, "Especialidad no encontrada")
...
def disponibilidad(medico_id: int, fecha: str, db: Session = Depends(get_db)):
    citas = db.query(Cita).filter(Cita.medico_id == medico_id,
        Cita.fecha == fecha, Cita.estado != "cancelada").all()
...
def proxima_cita(u=Depends(get_user), db: Session = Depends(get_db)):
    ...
    if u.get_rol() == "paciente": q = q.filter(Cita.paciente_id == u.id)
...
def get_cita(cita_id: int, u=Depends(get_user), db: Session = Depends(get_db)):
    ...
    return {"id": c.id, "paciente_nombre": c.paciente.nombre,
            ...
            "estado": c.get_estado(), "costo": c.costo,
            "notas_medico": c.notas_medico or ""}

@router.put("/citas/{cita_id}")
def update_cita(cita_id: int, d: CitaUpdateReq, u=Depends(get_user),
                db: Session = Depends(get_db)):
    c = db.query(Cita).filter(Cita.id == cita_id).first()
    if not c: raise HTTPException(404, "Cita no encontrada")
